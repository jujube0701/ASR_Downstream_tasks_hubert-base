...
def extract_features(model, feature_extractor, dataloader, device, save_interval=100):
    ...
    for i, (waveforms, label_ids) in enumerate(tqdm(dataloader)):
        ...
        if (i + 1) % save_interval == 0:
            part_path = f'UrbanSound8K_feature_fc/urbansound8k_features_hubert_part_{i+1}.pt'
...
def merge_feature_parts():
    import glob
    part_files = sorted(glob.glob('UrbanSound8K_feature_fc/urbansound8k_features_hubert_part_*.pt'), key=lambda x: int(x.split('_')[-1].split('.')[0]))
