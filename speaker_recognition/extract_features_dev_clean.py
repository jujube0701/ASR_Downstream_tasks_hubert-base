...
# ====== 參數區 ======
audio_root = 'G:/surf/hubert-base/LibriSpeech/dev-clean'
output_path = 'librispeech_dev_clean_features.pt'
batch_size = 1
# ====== END ======
    ...
    for i, (wav_path, speaker_id) in enumerate(tqdm(wav_list)):
        ...
        if (i + 1) % save_interval == 0:
            if features:  # 確保有特徵才保存
                part_path = os.path.join(os.path.dirname(output_path), f'librispeech_dev_clean_features_part_{i+1}.pt')
...
def merge_feature_parts():
    ...
    part_files = sorted(glob.glob(os.path.join(features_dir, 'librispeech_dev_clean_features_part_*.pt')), 
